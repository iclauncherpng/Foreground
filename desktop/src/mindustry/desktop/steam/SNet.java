...
public class SNet implements SteamNetworkingCallback, SteamMatchmakingCallback, SteamFriendsCallback, NetProvider{
    ...
    public void onLobbyEnter(SteamID steamIDLobby, int chatPermissions, boolean blocked, ChatRoomEnterResponse response){
        ...
        //check version
        if(!Version.build.equals(String.valueOf(version))){
            ui.loadfrag.hide();
            ui.showInfo("[scarlet]" + (version > Strings.parseInt(Version.build, 0) ? KickReason.clientOutdated : KickReason.serverOutdated) + "\n[]" +
        ...
        if(lobbyDoneCallback != null){
            ...
            for(int i = 0; i < matches; i++){
                try{
                    ...
                    if(mode == null || mode.isEmpty() || (!Version.build.equals("-1") && !smat.getLobbyData(lobby, "version").equals(Version.build))) continue;
                    ...
                    Host out = new Host(
                        ...
                        smat.getLobbyData(lobby, "version"),

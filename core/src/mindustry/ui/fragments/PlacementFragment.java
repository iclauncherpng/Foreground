            ...
            full.table(frame -> {
                ...
                //blockCatTable: all blocks | all categories
                {
                    ...
                    blockCatTable.table(categories -> {
                        final boolean[] lastCanModify = {state.rules.canModifyWorld};
                        final boolean[] lastExperiments = {state.rules.experiments};
                        ...
                        Seq<Category> visibleCats = new Seq<>();
                        for(Category cat : Category.all){
                            Seq<Block> blocks = getUnlockedByCategory(cat);
                            boolean empty = blocks.isEmpty();

                            if(cat == Category.terrain || cat == Category.stone){
                                if(!state.rules.canModifyWorld) continue;
                                empty = false;
                            }
                            if(cat == Category.experiments){
                                if(!state.rules.experiments) continue;
                                empty = false;
                            }
                            if(!empty) visibleCats.add(cat);
                        }

                        if(visibleCats.any() && !visibleCats.contains(currentCategory)){
                            currentCategory = visibleCats.first();
                        }

                        int f = 0;
                        for(Category cat : visibleCats){
                            if(f++ % 2 == 0) categories.row();

                            Drawable icon = ui.getIcon(cat.name());
                            if(icon == Icon.android || cat == Category.terrain) icon = Icon.terrain;
                            if(icon == Icon.android || cat == Category.stone) icon = Icon.box;
                            if(cat == Category.experiments) icon = Icon.commandRally;

                            categories.button(icon, Styles.clearTogglei, () -> {
                                currentCategory = cat;
                        ...
                        if(visibleCats.size % 2 != 0){
                            categories.image(Styles.black6).size(50f);
                        }
                    }).fillY().bottom().touchable(Touchable.enabled);
                }

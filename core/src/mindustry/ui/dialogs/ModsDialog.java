...
public class ModsDialog extends BaseDialog{
    ...
    private @Nullable String getStateText(LoadedMod item){
        if(!Version.isAtLeast(item.meta.minGameVersion)){
            return Core.bundle.format("mod.requiresversion", item.meta.minGameVersion);
        }

        if(item.meta.locked && !item.meta.minGameVersion.equals(Version.build)){
            return Core.bundle.format("mod.lockedmod", item.meta.minGameVersion);
        }
    ...
    private @Nullable String getStateDetails(LoadedMod item){
        if(item.isOutdated()) {
            return Core.bundle.format("mod.incompatiblemod.details", Version.build);
        }else if(item.meta.locked && !item.meta.minGameVersion.equals(Version.build)){
                return Core.bundle.get("mod.lockedmod.details");
        }else if(item.isBlacklisted()){
    ...
    private void showMod(LoadedMod mod){
        ...
        if(mod.getRepo() != null){
            ...
            dialog.buttons.button("@mods.github.open", Icon.link, () -> Core.app.openURI(mod.meta.repo));
        ...
        getModList(0, rlistings -> {
            ...
            for(ModListing mod : listings){
                ...
                browserTable.button(con -> {
                    ...
                    con.add(new BorderImage(){
                        ...
                        public void draw(){
                            ...
                            if(!textureCache.containsKey(repo)){
                                ...
                                String iconUrl = repo.startsWith("http") ?
                                        repo.replace("/blob/", "/").replace("github.com", "raw.githubusercontent.com") + "/master/icon.png" :
                    ...
                    (mod.locked && !mod.minGameVersion.equals(Version.build)) ? "\n" + Core.bundle.format("mod.requiresversion", mod.minGameVersion) :
                    ((mod.hasJava && !Version.isAtLeast(minJavaModGameVersion)) ? "\n" + Core.bundle.format("mod.requiresversion", mod.minGameVersion) : ""));
                    ...
                    sel.buttons.button("@mods.browser.view-releases", Icon.zoom, () -> {
                        ...
                        Http.get(ghApis[0] + "/repos/" + mod.repo + "/releases", res -> {
    ...
    public void githubImportMod(String repo, boolean isJava, @Nullable String release){
        githubImportMod(repo, isJava, release, 0);
    }

    private void githubImportMod(String repo, boolean isJava, @Nullable String release, int apiIndex){
        if(apiIndex >= ghApis.length) return;
        ...
        String cleanedRepo = repo;
        if(repo.startsWith("http")){
            String noProto = repo.substring(repo.indexOf("//") + 2);
            if(noProto.contains("/")){
                cleanedRepo = noProto.substring(noProto.indexOf("/") + 1);
            }
        }
        ...
        final String finalRepo = cleanedRepo;

        Http.get(ghApis[apiIndex] + "/repos/" + finalRepo, res -> {
            if(cancelledImport) return;
            var json = Jval.read(res.getResultAsString());

            String mainBranch = json.getString("default_branch", "main");
            ...
            if(language.equals("Java") || language.equals("Kotlin") || language.equals("Groovy") || language.equals("Scala")){
                githubImportJavaMod(finalRepo, release);
            }else{
                githubImportBranch(mainBranch, finalRepo, release);
            }
        }, error -> {
            if(apiIndex < ghApis.length - 1){
                githubImportMod(repo, isJava, release, apiIndex + 1);
            }else{
                importFail(error);
            }
        });
    ...
    public void importDependencies(Seq<String> dependencies, Runnable done){
        getModList(listings -> {
            listings.each(l -> dependencies.contains(l.internalName), l -> {
                ...
                githubImportMod(l.repo, l.hasJava, null);
            });
            done.run();
        });
    }
    ...
    private void githubImportJavaMod(String repo, @Nullable String release){
        Http.get(ghApis[0] + "/repos/" + repo + "/releases/" + (release == null ? "latest" : release), res -> {
            if(cancelledImport) return;
            var json = Jval.read(res.getResultAsString());
            ...
            if(asset != null){
                ...
                ui.showErrorMessage("No JAR file found in releases. Make sure you have a valid jar file in the mod's latest Github Release.");
            }
        }, this::importFail);
    }
    ...
    private void githubImportBranch(String branch, String repo, @Nullable String release){
        if(release != null) {
            Http.get(ghApis[0] + "/repos/" + repo + "/releases/" + release, res -> {
                if(cancelledImport) return;
            ...
            Http.get(ghApis[0] + "/repos/" + repo + "/zipball/" + branch, loc -> {
                if(cancelledImport) return;
                if(loc.getHeader("Location") != null){
                    Http.get(loc.getHeader("Location"), result -> {
                        if(cancelledImport) return;

...
public class CustomRulesDialog extends BaseDialog{
    ...
    void setupMain(){
        ...
        check("@rules.canmodifyworld", b -> rules.canModifyWorld = b, () -> rules.canModifyWorld);
        ...
        current.button("@rules.weather.reset", () -> {
            mindustry.gen.Groups.weather.each(mindustry.gen.WeatherState::remove);
            rules.weather.clear();
        }).left().width(250f).row();
    ...
    public void slideri(String text, Intc cons, Intp prov, int min, int max, int step, Func<Integer, String> sfunc, Boolp condition){
        if(!Core.bundle.get(text.substring(1)).toLowerCase().contains(ruleSearch)) return;
        current.table(t -> {
            t.left();
            t.add(text).left().padRight(5).update(a -> a.setColor(condition.get() ? Color.white : Color.gray));
            t.slider(min, max, step, prov.get(), f -> cons.get((int)f)).update(s -> {
                s.setDisabled(!condition.get());
                s.setValue(prov.get());
            }).width(150f);
            t.label(() -> sfunc.get(prov.get())).padLeft(10f).minWidth(40f);
        }).row();
    }

    ...
    public PlanetDialog(){
        ...
        shown(() -> {
            if(!settings.getBool("campaignselect") && !content.planets().contains(p -> p.sectors.contains(Sector::hasBase))){
                ...
                diag.cont.row();
                diag.cont.label(() -> selected[0] == null ? "@campaign.none" : "@campaign." + selected[0].name).labelAlign(Align.center).style(Styles.outlineLabel).width(440f).wrap().colspan(3);
        ...
        buttons,
                ...
                new Table(t -> {
                    ...
                    for(Planet star : content.planets()){
                        ...
                        horizontalGroup.table(Tex.pane, systemTable -> {
                            systemTable.margin(8f);
                            systemTable.align(Align.top);
                            systemTable.top();

                            systemTable.table(s -> {
                                s.image(Icon.icons.get(star.icon, Icon.commandRallySmall)).color(star.iconColor).padRight(4f);
                                s.add(star.localizedName).color(star.iconColor).left();
                            }).padBottom(8f).left().row();

                            systemTable.image().color(Pal.accent).fillX().height(2f).pad(4f).row();

                            for(Planet planet : content.planets()){
                                if(planet.solarSystem == star && selectable(planet)){
                                    Button planetButton = systemTable.button(planet.localizedName,
                                                    Icon.icons.get(planet.icon + "Small", Icon.icons.get(planet.icon, Icon.commandRallySmall)),
                                                    Styles.flatTogglet, () -> viewPlanet(planet, false)
                                            ).width(180).height(36).pad(2f).update(bb -> bb.setChecked(state.planet == planet))
                                            .with(w -> {
                                                w.marginLeft(8f);
                                                w.setBackground(Tex.pane);
                                            }).get();

                                    planetButton.setColor(planet.iconColor);
                                    planetButton.getChildren().get(1).setColor(planet.iconColor);
                                    planetButton.getChildren().get(0).setColor(Color.white);

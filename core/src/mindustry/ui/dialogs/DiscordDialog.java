...
public class DiscordDialog extends Dialog{
    ...
    public DiscordDialog(){
        ...
        cont.table(t -> {
            ...
            t.add("@discordnd").color(Pal.accent).growX().padLeft(10f);
        }).size(520f, h).pad(10f);
        ...
        buttons.button("@minlink", Icon.discord, () -> {
            if(!Core.app.openURI(discordURL)){
                ui.showErrorMessage("@linkfail");
                Core.app.setClipboardText(discordURL);
            }
        });
        buttons.button("@newminlink", Icon.discord, () -> {
            if(!Core.app.openURI(discordndURL)){
                ui.showErrorMessage("@linkfail");
                Core.app.setClipboardText(discordURL);
            }
        });

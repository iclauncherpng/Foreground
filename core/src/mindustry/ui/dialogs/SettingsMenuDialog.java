...
public class SettingsMenuDialog extends BaseDialog{
    ...
    void rebuildMenu(){
        ...
        menu.button("@settings.mods", Icon.book, style, isize, () -> {
            if(ui.mods != null) ui.mods.show();
        }).marginLeft(marg).disabled(!enableMods).row();
    ...
    void addSettings(){
        ...
        game.checkPref("coolsetting", false, val -> {
            if(Vars.grayscale != null){
                Vars.grayscale.active = val;
            }
        });
        ...
        graphics.sliderPref("territoryopacity", 100, 0, 100, 5, s -> s + "%");
        ...
        graphics.sliderPref("minmagnificationmultiplierpercent", 100, 100, 400, 25, s -> {
            if(ui.settings != null){
        ...
        graphics.checkPref("underwatershaders", true, value -> {
            renderer.drawUnderwater = value;
            Draw.flush();
        });

        graphics.checkPref("territoryrender", !mobile);

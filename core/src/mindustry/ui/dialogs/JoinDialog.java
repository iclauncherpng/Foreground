...
public class JoinDialog extends BaseDialog{
    ...
    void safeConnect(String ip, int port, String version){
        if(!Version.build.equals(version) && !Version.build.equals("-1") && !version.equals("-1")){
            ui.showInfo("[scarlet]" + Core.bundle.get("server.outdated") + "\n[]" +
    ...
    private String getVersionString(Host host){
        ...
        }else if(host.version.equals("0")){
            return Core.bundle.get("server.outdated");
        }
        if(Version.build.equals(host.version) && Version.type.equals(host.versionType)){
            return "";
        }

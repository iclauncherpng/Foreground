...
public class ErekirTechTree{
    ...
    public static void load(){
        ...
        if(Vars.devMode){
            content.blocks().each(b -> b.alwaysUnlocked = true);
            content.units().each(u -> u.alwaysUnlocked = true);
            content.sectors().each(s -> s.alwaysUnlocked = true);
        }
                ...
                node(ductRouter, () -> {
                    node(ductBridge, () -> {
                        node(armoredDuct, () -> {
                            node(surgeConveyor, () -> {
                                node(surgeRouter, () -> {
                                    node(phaseDuct, Seq.with(new Research(phaseSynthesizer)), () -> {

...
public class PlanetRenderer implements Disposable{
    ...
    public void render(PlanetParams params){
        updateOrbits(params.planet.solarSystem);
        Draw.flush();
...
//    private void updateOrbits(Planet planet){
//        if(planet.parent != null && planet.orbitTime > 0){
//            double unix = System.currentTimeMillis() / 1000.0;
//            double offlineOffset = unix % planet.orbitTime;
//            double smoothTime = offlineOffset + Time.globalTime;
//            float shift = Math.abs(planet.name.hashCode() % 360);
//            float speed = 360f / planet.orbitTime;
//            planet.orbitOffset = (float)((smoothTime * speed + shift) % 360f);
//        }
//
//        for(Planet child : planet.children) updateOrbits(child);
//    }

    private void updateOrbits(Planet planet){
        if(planet.parent != null && planet.orbitTime > 0){
            double totalSeconds = System.currentTimeMillis() / 1000.0;
            float startAngle = Math.abs(planet.name.hashCode() % 360);
            float speed = 360f / planet.orbitTime;
            planet.orbitOffset = (float)((totalSeconds * speed + startAngle) % 360.0);
        }

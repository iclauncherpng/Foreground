...
public class MinimapRenderer{
    ...
    public void drawEntities(float x, float y, float w, float h, boolean fullView){
        ...
        if(state.rules.diplomacy && TerritorySystem.initialized){
            Draw.flush();
            ...
            for(var data : state.teams.present){
                if(data.team == Team.derelict || data.buildings.isEmpty()) continue;

                Draw.color(data.team.color, 0.25f);

                data.buildings.each(b -> TerritorySystem.territoryBlocks.containsKey(b.block), b -> {
                    float radius = TerritorySystem.territoryBlocks.get(b.block, 0f);
                    float side = radius * 0.7071f * 2f;
                    float bx = b.x - (b.block.size % 2 == 0 ? 4f : 0f);
                    float by = b.y - (b.block.size % 2 == 0 ? 4f : 0f);

                    Fill.rect(bx, by, side, side);
                });

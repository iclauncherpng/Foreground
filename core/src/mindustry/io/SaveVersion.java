...
public abstract class SaveVersion extends SaveFileReader{
    ...
    public void read(DataInputStream stream, CounterInputStream counter, WorldContext context) throws IOException{
        ...
        try{
            ...
            if(stream.available() > 0){
                readRegion("diplomacy", stream, counter, in -> {
                    for(int i = 0; i < 256; i++){
                        in.readFully(Team.teamRelations[i]);
                    }
                });
            }
    ...
    public void write(DataOutputStream stream, StringMap extraTags) throws IOException{
        ...
        writeRegion("diplomacy", stream, out -> {
            for(int i = 0; i < 256; i++){
                out.write(Team.teamRelations[i]);
            }
        });
    }
...
//        }
    ...
    public void writeDiplomacy(DataOutput stream) throws IOException{
        for(int i = 0; i < 256; i++){
            stream.write(Team.teamRelations[i]);
        }
    }

    public void readDiplomacy(DataInput stream) throws IOException{
        for(int i = 0; i < 256; i++){
            stream.readFully(Team.teamRelations[i]);
        }
        state.teams.updateEnemies();
    }
}

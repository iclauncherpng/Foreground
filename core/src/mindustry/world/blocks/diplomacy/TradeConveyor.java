...
public class TradeConveyor extends StackConveyor {
    ...
    public class TradeConveyorBuild extends StackConveyorBuild {
        ...
        public boolean acceptItem(Building source, Item item){
            if(this == source) return items.total() < block.itemCapacity && (!items.any() || items.has(item));
            if(cooldown > recharge - 1f) return false;
            if(source instanceof TradeConveyorBuild) {
                return items.total() < block.itemCapacity && (!items.any() || items.has(item));
            }
            return items.total() < block.itemCapacity && (!items.any() || items.has(item));
        }
        ...
        public void onProximityUpdate(){
            ...
            if(forward instanceof TradeConveyorBuild){
                state = stateMove;
                if(!headless) blendprox |= 1;
            } else if(forward != null && forward.block.acceptsItems){
                state = stateUnload;
            } else {
                state = stateLoad;
            }

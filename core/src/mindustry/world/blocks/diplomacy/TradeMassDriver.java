...
public class TradeMassDriver extends Block{
    public float range;
    ...
    public TradeMassDriverBolt bullet = new TradeMassDriverBolt();
    ...
    public TradeMassDriver(String name){
        super(name);
        update = true;
        solid = true;
        configurable = true;
        ...
        //point2 is relative
        config(Point2.class, (TradeMassDriverBuild tile, Point2 point) -> tile.link = Point2.pack(point.x + tile.tileX(), point.y + tile.tileY()));
        config(Integer.class, (TradeMassDriverBuild tile, Integer point) -> tile.link = point);
    }
    ...
    public void setBars(){
        super.setBars();
        addBar("sendMode", (TradeMassDriverBuild entity) -> new Bar(
                () -> Core.bundle.get(entity.sendMode ? "trademassdriver.mode.1" : "trademassdriver.mode.2"),
                () -> entity.sendMode ? Pal.accent : Pal.remove,
                () -> entity.sendMode ? 1f : 1f
        ));
    }
    ...
    public static class TradeDriverBulletData implements Poolable{
        public TradeMassDriverBuild from, to;
    ...
    public class TradeMassDriverBuild extends Building implements RotBlock{
        ...
        public void buildConfiguration(Table table){
            table.button(sendMode ? Icon.upload : Icon.download, () -> {
                sendMode = !sendMode;
            }).size(50f).tooltip(sendMode ? "@trademassdriver.mode.1" : "@trademassdriver.mode.2");
        }
        ...
        public void updateTile(){
            ...
            if(state == DriverState.accepting){
                ...
                if(!sendMode || !hasLink || (!waitingShooters.isEmpty() && (itemCapacity - items.total() >= minDistribute))){
                    state = DriverState.idle;
                    return;
                }
                ...
                if(
                    ...
                    TradeMassDriverBuild other = (TradeMassDriverBuild)link;
        ...
        public boolean onConfigureBuildTapped(Building other){
            ...
            if(link == other.pos()){
                configure(-1);
                return false;
            }else if(other.block == block && other.dst(tile) <= range){
                configure(other.pos());
                return false;
            }
        ...
        protected void fire(TradeMassDriverBuild target){
            //reset reload, use power.
            reloadCounter = 1f;

            TradeDriverBulletData data = Pools.obtain(TradeDriverBulletData.class, TradeDriverBulletData::new);
            data.from = this;
            data.to = target;
            int totalUsed = 0;
        ...
        public void handlePayload(Bullet bullet, TradeDriverBulletData data){
        ...
        protected boolean shooterValid(Building other){
            return other instanceof TradeMassDriverBuild entity && other.isValid() && other.efficiency > 0 && entity.block == block && entity.link == pos() && within(other, range);
        }

        protected boolean linkValid(){
            if(link == -1) return false;
            return world.build(this.link) instanceof TradeMassDriverBuild other && other.block == block && within(other, range);
        }

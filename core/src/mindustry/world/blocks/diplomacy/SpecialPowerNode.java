...
public class SpecialPowerNode extends PowerBlock {
    ...
    public @Load(value = "@-tr-laser", fallback = "tr-laser") TextureRegion laser;
    public @Load(value = "@-tr-laser-end", fallback = "tr-laser-end") TextureRegion laserEnd;
    ...
    public SpecialPowerNode(String name){
        super(name);
        configurable = true;
        consumesPower = false;
        outputsPower = false;
    ...
    protected void getPotentialLinks(Tile tile, Team team, Cons<Building> others){
        ...
        Boolf<Building> valid = other -> other != null && other.tile != tile && other.block.connectedPower && other.power != null &&
            (other.block.outputsPower || other.block.consumesPower || other.block instanceof SpecialPowerNode) &&
            overlaps(tile.x * tilesize + offset, tile.y * tilesize + offset, other.tile, laserRange * tilesize) && other.team == team &&
            !graphs.contains(other.power.graph) &&
            !SpecialPowerNode.insulated(tile, other.tile) &&
            !(other instanceof PowerNodeBuild obuild && obuild.power.links.size >= ((SpecialPowerNode)obuild.block).maxNodes) &&
        ...
        tempBuilds.sort((a, b) -> {
            int type = -Boolean.compare(a.block instanceof SpecialPowerNode, b.block instanceof SpecialPowerNode);
    ...
    public static void getNodeLinks(Tile tile, Block block, Team team, Cons<Building> others){
        Boolf<Building> valid = other -> other != null && other.tile != tile && other.block instanceof SpecialPowerNode node &&
        node.autolink &&
    ...
    public boolean linkValid(Building tile, Building link, boolean checkMaxNodes){
        ...
        if(overlaps(tile, link, laserRange * tilesize) || (link.block instanceof SpecialPowerNode node && overlaps(link, tile, node.laserRange * tilesize))){
            if(checkMaxNodes && link.block instanceof SpecialPowerNode node){
    ...
    public class PowerNodeBuild extends Building{
        ...
        public void draw(){
            ...
            for(int i = 0; i < power.links.size; i++){
                ...
                if(link.block instanceof SpecialPowerNode && link.id >= id) continue;

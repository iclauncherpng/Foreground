...
public class TradeBridge extends Block{
    ...
    //for autolink
    public @Nullable TradeBridgeBuild lastBuild;

    public TradeBridge(String name){
        super(name);
        update = true;
        solid = true;
        underBullets = true;
        ...
        config(Point2.class, (TradeBridgeBuild tile, Point2 i) -> tile.link = Point2.pack(i.x + tile.tileX(), i.y + tile.tileY()));
        //integer is not
        config(Integer.class, (TradeBridgeBuild tile, Integer i) -> tile.link = i);
    }
    ...
    public static class BridgeNetwork {
        public final Seq<TradeBridgeBuild> builds = new Seq<>();

        public boolean hasPower() {
            for (TradeBridgeBuild build : builds) {
                if (build.power != null &&
                        build.power.graph != null &&
                        build.power.graph.getPowerBalance() >= 0 &&
                        build.power.graph.getPowerProduced() > 0) {
                    return true;
                }
            }
            return false;
        ...
        public void add(TradeBridgeBuild build) {
    ...
    public boolean linkValid(Tile tile, Tile other, boolean checkDouble){
        ...
        return ((other.block() == tile.block() && tile.block() == this) || (!(tile.block() instanceof TradeBridge) && other.block() == this))
                && (other.team() == tile.team() || tile.block() != this)
                && (!checkDouble || ((TradeBridgeBuild)other.build).link != tile.pos());
    }
    ...
    public class TradeBridgeBuild extends Building{
        public int link = -1;
        public @Nullable BridgeNetwork network;
        ...
        public void updateNetwork() {
            ...
            BridgeNetwork net = new BridgeNetwork();
            Seq<TradeBridgeBuild> queue = new Seq<>();
            Seq<TradeBridgeBuild> visited = new Seq<>();

            queue.add(this);
            visited.add(this);

            while (queue.size > 0) {
                TradeBridgeBuild current = queue.remove(0);

                
                Tile other = world.tile(current.link);
                if (linkValid(current.tile, other) && other.build instanceof TradeBridgeBuild b) {
                ...
                for (int i = 0; i < current.incoming.size; i++) {
                    Tile incTile = world.tile(current.incoming.items[i]);
                    if (linkValid(current.tile, incTile, false) && incTile.build instanceof TradeBridgeBuild b) {
                        if (!visited.contains(b)) {
                            visited.add(b);
                            queue.add(b);
                        }
            ...
            for (TradeBridgeBuild b : visited) {
                net.builds.add(b);
                b.network = net;
            }
        }
        ...
        public boolean onConfigureBuildTapped(Building other){
            //reverse connection
            if(other instanceof TradeBridgeBuild b && b.link == pos()){
                configure(other.pos());
                other.configure(-1);
                return true;
            }
        ...
        public void checkIncoming(){
            ...
            while(idx < incoming.size){
                ...
                if(!linkValid(tile, other, false) || ((TradeBridgeBuild)other.build).link != tile.pos()){
        ...
        public void updateTile(){
            ...
            if (network == null || timer(timerCheckMoved, 60f)) {
                updateNetwork();
            }
            ...
            if(!linkValid(tile, other)){
                ...
                var inc = ((TradeBridgeBuild)other.build).incoming;
                int pos = tile.pos();
                if(!inc.contains(pos)){
                    inc.add(pos);
                }
                ...
                boolean powered = efficiency > 0 &&
                        ((network != null && network.hasPower()) ||
                                (power != null && power.graph != null && power.graph.getPowerBalance() >= 0));

                warmup = Mathf.approachDelta(warmup, powered ? efficiency : 0f, 1f / 30f);
        ...
        protected boolean linked(Building source){
            return source instanceof TradeBridgeBuild && linkValid(source.tile, tile) && ((TradeBridgeBuild)source).link == pos();
        }
        ...
        public void onRemoved() {
            super.onRemoved();
            if (network != null) {
                network.builds.remove(this);
                for (TradeBridgeBuild b : network.builds) {
                    b.network = null;
                }
            }
        }

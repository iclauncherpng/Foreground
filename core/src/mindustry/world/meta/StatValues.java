...
public class StatValues{
    ...
    public static <T extends UnlockableContent> StatValue orderedAmmo(ObjectMap<T, BulletType> map, String blockName){
        return table -> {
            table.row();
            ...
            for(T t : map.keys()){
                ...
                if(type.spawnUnit != null && type.spawnUnit.weapons.size > 0){
                    ammo(ObjectMap.of(t, type.spawnUnit.weapons.first().bullet), false, false, blockName).display(table);
                    continue;
                }
                ...
                table.table(Styles.grayPanel, bt -> {
                    ...
                    if(type.buildingDamageMultiplier != 1) sep(bt, Core.bundle.format("bullet.buildingdamage", ammoStat((int)(type.buildingDamageMultiplier * 100 - 100))));
                    if(type.rangeChange != 0 && !compact) sep(bt, Core.bundle.format("bullet.range", ammoStat(type.rangeChange / tilesize)));
                    if(type.shieldDamageMultiplier != 1) sep(bt, Core.bundle.format("bullet.shielddamage", ammoStat((int)(type.shieldDamageMultiplier * 100 - 100))));
                    if(type.splashDamage > 0) sep(bt, Core.bundle.format("bullet.splashdamage", (int)type.splashDamage, Strings.fixed(type.splashDamageRadius / tilesize, 1)));
                    if(type.statLiquidConsumed <= 0f && !compact && !Mathf.equal(type.ammoMultiplier, 1f) && type.displayAmmoMultiplier && (!(t instanceof Turret turret) || turret.displayAmmoMultiplier)) sep(bt, Core.bundle.format("bullet.multiplier", (int)type.ammoMultiplier));
                    if(!compact && !Mathf.equal(type.reloadMultiplier, 1f)) sep(bt, Core.bundle.format("bullet.reload", ammoStat((int)(type.reloadMultiplier * 100 - 100))));
                    if(type.knockback > 0) sep(bt, Core.bundle.format("bullet.knockback", Strings.autoFixed(type.knockback, 2)));
                    if(type.healPercent > 0f) sep(bt, Core.bundle.format("bullet.healpercent", Strings.autoFixed(type.healPercent, 2)));
                    if(type.healAmount > 0f) sep(bt, Core.bundle.format("bullet.healamount", Strings.autoFixed(type.healAmount, 2)));
                    if(type.pierce || type.pierceCap != -1) sep(bt, type.pierceCap == -1 ? "@bullet.infinitepierce" : Core.bundle.format("bullet.pierce", type.pierceCap));
                    if(type.incendAmount > 0) sep(bt, "@bullet.incendiary");
                    if(type.homingPower > 0.01f) sep(bt, "@bullet.homing");
                    if(type.lightning > 0) sep(bt, Core.bundle.format("bullet.lightning", type.lightning, type.lightningDamage < 0 ? type.damage : type.lightningDamage));

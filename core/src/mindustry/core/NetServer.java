...
public class NetServer implements ApplicationListener{
    ...
    private static ObjectMap<String, Byte> diplomacyRequests = new ObjectMap<>();
    ...
    public TeamAssigner assigner = (player, players) -> {
        if(state.rules.diplomacy){
            var activeTeams = state.teams.getActive().select(data ->
                    data.team != Team.derelict &&
                            data.hasCore() &&
                            data.team.rules().protectCores
            );

            if(activeTeams.isEmpty()) return state.rules.defaultTeam;

            TeamData best = activeTeams.min(data -> {
                int count = 0;
                for(Player p : players){
                    if(p.team() == data.team && p != player) count++;
                }
                return (float)count + (data.team.id * 0.001f);
            });
    ...
    public NetServer(){
        ...
        net.handleServer(ConnectPacket.class, (con, packet) -> {
            ...
            if(packet.versionType == null || packet.version == null || (
                    (packet.version.equals("-1") || !packet.versionType.equals(Version.type) || !packet.version.equals(Version.build))
                            && !Version.build.equals("-1")
                            && !admins.allowsCustomClients()
            )){
            ...
            if(!Version.build.equals(packet.version) && !Version.build.equals("-1") && !packet.version.equals("-1")){
                con.kick(KickReason.clientOutdated);
                return;
            }

            if(packet.version.equals("-1")){
                con.modclient = true;
            }
    ...
    private void registerCommands(){
        ...
        clientCommands.<Player>register("diplomacy", "<toID> <relation>", "Diplomacy Sync", (args, player) -> {
            try {
                int toId = Strings.parseInt(args[0]);
                String relName = args[1].toLowerCase();
                int fromId = player.team().id;
                Team.set(fromId, toId, relName);
                Call.setRules(state.rules);
                state.teams.updateEnemies();

                Log.info("Diplomacy: @ set @ to @", player.name, toId, relName);
            } catch (Exception e) {
                player.sendMessage("[scarlet]Error. Usage: /diplomacy <ID> <status>");
            }
        });

        clientCommands.<Player>register("diplomacy", "<toID> <relation>", "Diplomacy request", (args, player) -> {
            int fromId = player.team().id;
            int toId = Strings.parseInt(args[0]);
            String relName = args[1].toLowerCase();
            Team.Relation requestedRel = Team.Relation.valueOf(relName);

            if(fromId == toId) return;

            if(requestedRel == Team.Relation.enemy){
                Team.set(fromId, toId, "enemy");
                Call.sendMessage("[scarlet]ATTENTION! Fraction [#" + player.team().color + "]" + player.team().localized() + " [scarlet]declared war on the team [#" + Team.get(toId).color + "]" + Team.get(toId).localized() + "!");
                Call.setRules(state.rules);
                return;
            }

            String myKey = fromId + ":" + toId;
            String theirKey = toId + ":" + fromId;

            if(diplomacyRequests.get(theirKey, (byte)-1) == (byte)requestedRel.ordinal()){
                Team.set(fromId, toId, relName);
                diplomacyRequests.remove(theirKey);
                diplomacyRequests.remove(myKey);

                Call.sendMessage("[accent]Diplomacy: [white]Команды [#" + player.team().color + "]" + player.team().localized() + " [white]and [#" + Team.get(toId).color + "]" + Team.get(toId).localized() + " now [orange]" + relName + "!");
                Call.setRules(state.rules);
            } else {
                diplomacyRequests.put(myKey, (byte)requestedRel.ordinal());

                player.sendMessage("[yellow]Your request has been sent. Please wait for confirmation from the team. " + Team.get(toId).localized());

                Groups.player.each(p -> p.team().id == toId, p -> {
                    p.sendMessage("[orange]Fraction [#" + player.team().color + "]" + player.team().localized() + " [orange]offers: [white]" + relName.toUpperCase());
                    p.sendMessage("[lightgray]To agree, select the same status in the Diplomacy menu.");
                });
            }
        });
    }
